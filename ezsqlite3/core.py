...
LOG = logging.getLogger("ezsqlite3")
...
class Connection(Thread):
    def __init__(
        ...
        if loop is not None:
            warn(
                "ezsqlite3.Connection no longer uses the `loop` parameter",
                DeprecationWarning,
            )
    ...
    async def cursor(self) -> Cursor:
        """Create an ezsqlite3 cursor wrapping a sqlite3 cursor object."""
    ...
    async def backup(
        ...
        *,
        ...
        Takes either a standard sqlite3 or ezsqlite3 Connection object as the target.
        """
...
def connect(
    ...
    **kwargs: Any,
    ...
    if loop is not None:
        warn(
            "ezsqlite3.connect() no longer uses the `loop` parameter",
            DeprecationWarning,
        )
