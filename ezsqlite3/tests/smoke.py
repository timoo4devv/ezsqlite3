...
class SmokeTest(TestCase):
    ...
    async def test_connection_await(self):
        db = await ezsqlite3.connect(TEST_DB)
        self.assertIsInstance(db, ezsqlite3.Connection)
    ...
    async def test_connection_context(self):
        async with ezsqlite3.connect(TEST_DB) as db:
            self.assertIsInstance(db, ezsqlite3.Connection)
    ...
    async def test_connection_locations(self):
        ...
        for loc in locs:
            async with ezsqlite3.connect(loc) as db:
                cursor = await db.execute("select * from foo")
    ...
    async def test_multi_loop_usage(self):
        ...
        async with ezsqlite3.connect(":memory:") as db:
    ...
    async def test_connection_properties(self):
        async with ezsqlite3.connect(TEST_DB) as db:
            ...
            async with db.cursor() as cursor:
                cursor.row_factory = ezsqlite3.Row
                self.assertEqual(cursor.row_factory, ezsqlite3.Row)
                await cursor.execute("select * from test_properties")
                row = await cursor.fetchone()
                self.assertIsInstance(row, ezsqlite3.Row)
                self.assertEqual(row[1], 1)
                self.assertEqual(row[2], "hi")
                self.assertEqual(row["k"], 1)
                self.assertEqual(row["d"], "hi")

            db.row_factory = ezsqlite3.Row
            db.text_factory = bytes
            self.assertEqual(db.row_factory, ezsqlite3.Row)
    ...
    async def test_connect_error(self):
        ...
        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            async with ezsqlite3.connect(bad_db) as db:
                self.assertIsNone(db)  # should never be reached

        with self.assertRaisesRegex(OperationalError, "unable to open database"):
            await ezsqlite3.connect(bad_db)
    ...
    async def test_backup_ezsqlite3(self):
        def progress(a, b, c):
            print(a, b, c)

        async with ezsqlite3.connect(":memory:") as db1, ezsqlite3.connect(
            ":memory:"
        ) as db2:
    ...
    async def test_backup_sqlite(self):
        async with ezsqlite3.connect(":memory:") as db1:
